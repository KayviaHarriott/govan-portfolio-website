...
export const CustomCard: React.FC<CustomCardProps> = ({ label, text, url, version, title }) => {
  switch (version) {
    case "large":
      return (<Box
        sx={{
    ...
    case "medium":
      ...
      >
        <div className="flex flex-col gap-[24px]">
          <div>
            <h2 className="text-[32px] text-[#263238] font-[700]">{label}</h2>
        ...
        <div className="flex justify-end">
          <Link to={url} className="flex gap-[8px] text-[#FB5B03]">
            <p className="font-[700]">Portfolio</p> <ArrowForwardIcon sx={{ color: "##FB5B03" }} />
          </Link>
        </div>
      </Box>)
    break
